...
public final class PreviewEntityLevel {
    ...
    static @Nullable Entity createPlayer(@Nullable Level currentLevel) {
        ...
        try {
            RemotePlayer player = new RemotePlayer(level, previewProfile());
            player.setId(nextEntityId++);
            previewEntities.add(player);
            return player;
        } catch (Throwable exception) {
    ...
    private static DimensionType withoutTimelines(DimensionType type) {
        return new DimensionType(type.hasFixedTime(), type.hasSkyLight(), type.hasCeiling(), type.coordinateScale(),
                type.minY(), type.height(), type.logicalHeight(), type.infiniburn(), type.ambientLight(),
                type.monsterSettings(), type.skybox(), type.cardinalLightType(), type.attributes(), HolderSet.empty());
    }
