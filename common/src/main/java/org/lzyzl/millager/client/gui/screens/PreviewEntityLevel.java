...
public final class PreviewEntityLevel {
    ...
    private static final Set<Entity> previewEntities = Collections.synchronizedSet(
            Collections.newSetFromMap(new IdentityHashMap<>()));
    private static final Set<EntityType<?>> failedCreationTypes = Collections.newSetFromMap(new IdentityHashMap<>());
    private static final Set<EntityType<?>> failedRenderTypes = Collections.newSetFromMap(new IdentityHashMap<>());
    private static final ThreadLocal<Boolean> creatingPreviewEntity = new ThreadLocal<>();
    ...
    static @Nullable Entity create(EntityType<?> type, @Nullable Level currentLevel) {
        ...
        try {
            ...
            if (failedCreationTypes.add(type)) {
                Millager.LOGGER.error("Failed to create target relation preview for {}", type, exception);
            }
            return null;
        }
    }
    ...
    public static boolean isPreview(Entity entity) {
        return Boolean.TRUE.equals(creatingPreviewEntity.get()) || previewEntities.contains(entity);
    }
    ...
    private static <T extends Entity> @Nullable T createEntity(EntityType<T> type, Level level) {
        ...
        T entity;
        creatingPreviewEntity.set(true);
        try {
            entity = factory.create(type, level);
        } finally {
            creatingPreviewEntity.remove();
        }
    ...
    private static DimensionType withoutTimelines(DimensionType type) {
        return new DimensionType(type.hasFixedTime(), type.hasSkyLight(), type.hasCeiling(),
                ...
                HolderSet.empty(), type.ambientLight(), type.monsterSettings(), type.skybox(), type.cardinalLightType(),
    ...
    private static @Nullable ClientLevel createLevel(RegistryAccess.Frozen registryAccess) {
        try {
            ...
            WorldSessionTelemetryManager telemetry = new WorldSessionTelemetryManager(
                    TelemetryEventSender.DISABLED, false, Duration.ZERO, "millager-preview", profile.id());
            ...
            return new ClientLevel(connection, levelData, Level.OVERWORLD,
                    ...
                    2, 2, minecraft.levelExtractor, false, 0L, 63);
        } catch (Throwable exception) {
