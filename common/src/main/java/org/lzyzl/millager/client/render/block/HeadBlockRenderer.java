...
public class HeadBlockRenderer implements BlockEntityRenderer<HeadBlockEntity, SkullBlockRenderState> {
    ...
    public void extractRenderState(@NonNull HeadBlockEntity headBlockEntity, @NonNull SkullBlockRenderState skullBlockRenderState, float f, @NonNull Vec3 vec3, ModelFeatureRenderer.@Nullable CrumblingOverlay crumblingOverlay) {
        ...
        if (blockState.getBlock() instanceof WallSkullBlock) {
            ...
            skullBlockRenderState.transformation = SkullBlockRenderer.TRANSFORMATIONS.wallTransformation(facing);
        } else {
            skullBlockRenderState.transformation = SkullBlockRenderer.TRANSFORMATIONS.freeTransformations(blockState.getValue(SkullBlock.ROTATION));
        }
    ...
    public void submit(SkullBlockRenderState skullBlockRenderState, @NonNull PoseStack poseStack, @NonNull SubmitNodeCollector submitNodeCollector, @NonNull CameraRenderState cameraRenderState) {
        ...
        poseStack.pushPose();
        poseStack.mulPose(skullBlockRenderState.transformation);
        SkullBlockRenderer.submitSkull(skullBlockRenderState.animationProgress, poseStack, submitNodeCollector, skullBlockRenderState.lightCoords, skullModelBase, skullBlockRenderState.renderType, 0, skullBlockRenderState.breakProgress);
        poseStack.popPose();
    }
