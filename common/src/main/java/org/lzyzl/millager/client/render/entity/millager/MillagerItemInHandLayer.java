...
public class MillagerItemInHandLayer<T extends AbstractMillager> extends ItemInHandLayer<T, MillagerModel<T>> {
    ...
    @Override
    public void render(@NonNull PoseStack poseStack, @NonNull MultiBufferSource buffer, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
    ...
    @Override
    protected void renderArmWithItem(@NonNull LivingEntity entity, ItemStack stack, @NonNull ItemDisplayContext displayContext,
                                     @NonNull HumanoidArm arm, @NonNull PoseStack poseStack, @NonNull MultiBufferSource buffer, int packedLight) {
