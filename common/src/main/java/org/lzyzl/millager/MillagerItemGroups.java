...
public class MillagerItemGroups {
    ...
    public static final ResourceKey<CreativeModeTab> MILLAGER_CREATIVE_TAB_KEY = ResourceKey.create(
            ...
            ResourceLocation.fromNamespaceAndPath(Millager.MOD_ID, "creative_tab")
    );
    ...
    public static void addExtraContents(Consumer<ItemStack> output, CreativeModeTab.ItemDisplayParameters params) {
        ...
        params.holders().lookup(Registries.PAINTING_VARIANT).flatMap(lookup ->
            lookup.get(ResourceVariantHelper.COCKTAIL_TUTORIAL)).ifPresent(reference -> {
            RegistryOps<Tag> registryOps = params.holders().createSerializationContext(NbtOps.INSTANCE);
