...
public class MillagerRaidsData extends SavedData {
    ...
    public static MillagerRaidsData getOrCreate(ServerLevel level) {
        return level.getDataStorage().computeIfAbsent(
                new SavedData.Factory<>(MillagerRaidsData::new, MillagerRaidsData::load, DataFixTypes.SAVED_DATA_RAIDS),
                FILE_ID
        );
    }

    public static MillagerRaidsData load(CompoundTag tag, HolderLookup.Provider registries) {
        return CODEC.parse(NbtOps.INSTANCE, tag).result().orElseGet(MillagerRaidsData::new);
    }
