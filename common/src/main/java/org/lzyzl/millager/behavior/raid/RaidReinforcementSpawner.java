...
public final class RaidReinforcementSpawner {
    ...
    private static int spawnSquad(ServerLevel level, BlockPos center, BlockPos pos, RandomSource random,
        ...
        for (int i = 0; i < count; i++) {
            ...
            entity.setPos(memberPos.getX() + 0.5D, memberPos.getY(), memberPos.getZ() + 0.5D);
    ...
    private static void ensureCavalryHorseArmor(AbstractMillager entity, RandomSource random) {
        if (entity.getVehicle() instanceof Horse horse && horse.getItemBySlot(EquipmentSlot.BODY).isEmpty()) {
            horse.setItemSlot(EquipmentSlot.BODY, new ItemStack(Rider.getRandomHorseArmor(random, 3)));
        }
    }
