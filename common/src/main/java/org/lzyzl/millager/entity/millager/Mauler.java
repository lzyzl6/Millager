...
public class Mauler extends AbstractMillager {
    ...
    public boolean hurt(@NonNull DamageSource damageSource, float f) {
        ...
        if(isHurt && Objects.requireNonNull(this.getAttribute(Attributes.ARMOR)).getBaseValue() > 10) {
            ...
            else if(f >= 3.5 + this.level().getDifficulty().getId() * 0.5) {
    ...
    public void readAdditionalSaveData(@NonNull CompoundTag valueInput) {
        ...
        this.setFixing(valueInput.getBoolean("isFixing"));
        this.setFixingTicks(valueInput.getInt("fixingTicks"));
        this.setFixCooldown(valueInput.getInt("fixCooldown"));
    }
