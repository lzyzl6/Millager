...
public class InfantryPatrolGoal extends AbstractMillagerPatrolGoal {
    ...
    private LivingEntity findNearestEnemy() {
        if (!(this.mob.level() instanceof ServerLevel)) return null;
        ...
        candidates.addAll(this.mob.level().getEntitiesOfClass(
                ...
                player -> {
                    ...
                    if (this.mob instanceof NeutralMob neutral && neutral.getRemainingPersistentAngerTime() > 0 && player.getUUID().equals(neutral.getPersistentAngerTarget())) return true;
