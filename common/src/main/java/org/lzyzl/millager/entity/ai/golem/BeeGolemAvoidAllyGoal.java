...
public class BeeGolemAvoidAllyGoal extends TargetGoal {
    ...
    public boolean canUse() {
        ...
        for (LivingEntity entity : this.golem.level().getEntitiesOfClass(LivingEntity.class, target.getBoundingBox().inflate(6.0D))) {
            if (!MillagerTargetingHelper.hasBeeGolemOverride(entity) && this.golem.isAlliedTo(entity) && target.distanceTo(entity) < 1.5D) {
                return true;
            }
        }
        return false;
    ...
    public void start() {
        ...
        List<LivingEntity> monsters = this.golem.level().getEntitiesOfClass(LivingEntity.class, this.golem.getBoundingBox().inflate(20.0D),
                e -> this.golem.hasLineOfSight(e) && MillagerTargetingHelper.canBeeGolemAttack(e)
                        && (MillagerTargetingHelper.hasBeeGolemOverride(e) || !(e instanceof Creeper)) && e.isAlive()
                        && (MillagerTargetingHelper.hasBeeGolemOverride(e) || !this.golem.isAlliedTo(e) && this.golem.canAttack(e)));
        ...
        for (LivingEntity monster : monsters) {
            if (monster != currentTarget && monster.isAlive()) {
                ...
                for (LivingEntity entity : this.golem.level().getEntitiesOfClass(LivingEntity.class, monster.getBoundingBox().inflate(5.0D))) {
                    if (!MillagerTargetingHelper.hasBeeGolemOverride(entity) && this.golem.isAlliedTo(entity) && monster.distanceTo(entity) < 1.5D) {
                        safe = false;
                        break;
                    }
                }
    ...
    public void tick() {
        ...
        for (LivingEntity entity : this.golem.level().getEntitiesOfClass(LivingEntity.class, this.golem.getBoundingBox().inflate(10.0D))) {
            if (!MillagerTargetingHelper.hasBeeGolemOverride(entity) && this.golem.isAlliedTo(entity) && this.golem.distanceTo(entity) < 5.0D) {
