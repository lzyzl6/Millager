...
public class LancerSpearUseGoal<T extends AbstractMillager> extends Goal {
        ...
        if (!this.state.tickAndCheckFleeing()) {
            if (this.state.awayPos != null) {
                ...
                if (this.mob.getNavigation().isDone()) {
                    if (this.state.fleeingTime > 0) this.state.done = true;
                    else this.state.awayPos = null;
                }
            } else {
    ...
    private static class SpearUseState {
        ...
        private boolean notEngagedYet() { return this.engageTime < 0; }
        private void startEngagement(int ticks) { this.engageTime = ticks; }
