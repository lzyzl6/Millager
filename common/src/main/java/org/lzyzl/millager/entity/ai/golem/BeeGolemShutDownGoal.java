        ...
        if(beeGolem.level() instanceof ServerLevel level) {
            if (beeGolem.getLifeTicks() >= 2400) {
                ...
                level.sendParticles(ParticleTypes.GLOW,
                        ...
                        0.5, 0.5, 0.5,
                        0.1);
                level.playSound(beeGolem, beeGolem.getX(), beeGolem.getY(), beeGolem.getZ(), MillagerSounds.BEE_GOLEM_GLASS_BREAK, SoundSource.NEUTRAL, 2.0F, 0.5f);
                level.playSound(beeGolem, beeGolem.getX(), beeGolem.getY(), beeGolem.getZ(), MillagerSounds.BEE_GOLEM_WAX_BREAK, SoundSource.NEUTRAL, 2.0F, 0.5f);
                beeGolem.discard();
                return;
            }
            ...
            level.sendParticles(new DustParticleOptions(33980, 0.8f),
                    beeGolem.getX() + tremble, beeGolem.getY() + tremble + 0.2, beeGolem.getZ() + tremble, 1, 0, 0, 0, 0.01);
            level.sendParticles(new DustParticleOptions(5724249, 0.8f),
