...
public class Archer extends AbstractMillager implements RangedAttackMob {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(IS_CRAFTING, false);
    }
    ...
    @Override
    protected void dropCustomDeathLoot(@NonNull DamageSource damageSource, int lootingLevel, boolean recentlyHit) {
    ...
    public SpawnGroupData finalizeSpawn(
            ServerLevelAccessor level, @NonNull DifficultyInstance difficulty, @NonNull MobSpawnType entitySpawnReason,
            @Nullable SpawnGroupData spawnGroupData, @Nullable CompoundTag tag
    ) {
        RandomSource randomSource = level.getRandom();
        this.populateDefaultEquipmentSlots(randomSource, difficulty);
        this.enchantSpawnedWeapon(randomSource, difficulty.getSpecialMultiplier());
        return super.finalizeSpawn(level, difficulty, entitySpawnReason, spawnGroupData, tag);
    }
    ...
    protected void populateDefaultEquipmentSlots(@NonNull RandomSource random, DifficultyInstance difficulty) {
        if (difficulty.isHarderThan(1) && random.nextInt(11 - difficulty.getDifficulty().getId()) == 0) {
            ...
            if(random.nextInt(12) < 6 + difficulty.getDifficulty().getId()) {
                ...
                PotionUtils.setPotion(itemStack, random.nextBoolean() ? Potions.STRONG_POISON : Potions.STRONG_SLOWNESS);
    ...
    public void pickUpItem(@NonNull ItemEntity itemEntity) {
        ...
        if (this.wantsItem(itemStack)) {
            ...
            } else if (ItemStack.isSameItemSameTags(itemStack, offHand)) {
