...
public class ScouterTootHornGoal extends Goal {
    ...
    @Override
    public void stop() {
        if (this.scouter.level() instanceof ServerLevel sl) this.addEffectToAlly(sl);
    ...
    private void addEffectToAlly(ServerLevel serverLevel) {
        ...
        for (LivingEntity target : targets) {
            target.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 1200,0));
            target.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 1200,0));
            serverLevel.sendParticles(ParticleTypes.HAPPY_VILLAGER, target.getX(), target.getY() + 1.5D, target.getZ(), 3, 0.2, 0.2, 0.2, 0.1);
        }
    }
