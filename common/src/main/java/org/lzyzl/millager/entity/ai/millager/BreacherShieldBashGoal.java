...
public class BreacherShieldBashGoal extends Goal {
    ...
    public void tick() {
        ...
        for (LivingEntity victim : level.getEntitiesOfClass(LivingEntity.class, bashArea, this::isValidVictim)) {
            ...
            victim.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 100, 0), this.breacher);
        }
        MobEffectInstance strength = this.breacher.getEffect(MobEffects.DAMAGE_BOOST);
        this.breacher.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, strength == null ? 240 : strength.getDuration(), strength == null ? 1 : strength.getAmplifier() + 1), this.breacher);
