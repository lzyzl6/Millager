...
public abstract class AbstractMillager extends PathfinderMob implements NeutralMob, InventoryCarrier {
    ...
    public boolean hurtServer(@NonNull ServerLevel serverLevel, @NonNull DamageSource damageSource, float amount) {
        ...
        if (hurt && damageSource.getEntity() instanceof Player player) this.recordPlayerAttack(player, amount);
        return hurt;
    }
    ...
    public @Nullable AbstractMillager findNearbySquadLeader() {
        ...
        return this.level().getEntitiesOfClass(
                ...
                .stream()
                .min(Comparator.comparing(AbstractMillager::getUUID))
                .orElse(null);
    }

    public boolean hasNearbySquadCapacity() {
        UUID squadId = this.squadId;
        return squadId != null && this.level().getEntitiesOfClass(
                AbstractMillager.class,
                this.getBoundingBox().inflate(SQUAD_LEADER_SEARCH_RADIUS),
                entity -> squadId.equals(entity.getSquadId())
        ).size() < MAX_SQUAD_SIZE;
    }
    ...
    private void mergeNearbySquad() {
        ...
        AbstractMillager other = this.level().getEntitiesOfClass(AbstractMillager.class, this.getBoundingBox().inflate(SQUAD_MERGE_RADIUS), candidate -> candidate != this && candidate.isAlive() && candidate.isSquadLeader() && !squadId.equals(candidate.getSquadId()) && this.hasMatchingSquadCenter(candidate)).stream().min(Comparator.comparingDouble(this::distanceToSqr)).orElse(null);
        if (other == null || other.getSquadId() == null) return;
        UUID otherId = other.getSquadId();
        var members = this.level().getEntitiesOfClass(AbstractMillager.class, this.getBoundingBox().inflate(SQUAD_LEADER_SEARCH_RADIUS), candidate -> squadId.equals(candidate.getSquadId()) || otherId.equals(candidate.getSquadId()));
