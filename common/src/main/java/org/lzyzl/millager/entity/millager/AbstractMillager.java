...
public abstract class AbstractMillager extends PathfinderMob implements NeutralMob, InventoryCarrier {
    ...
    protected void setCanOpenDoors(boolean canOpenDoors) {
    ...
    public void tick() {
        ...
        if (this instanceof Rider && this.getVehicle() instanceof LivingEntity vehicle) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Creeper.class,
                6.0F, 1.3D, 1.5D, living -> !living.isRemoved() && living.isAlive()));
        ...
        this.targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(this, Player.class, 10, true, false, livingEntity -> {
            if (livingEntity instanceof Player player) {
                ...
                if (player.getY() <= -20 && this.level() instanceof ServerLevel serverLevel
                        && serverLevel.registryAccess().lookupOrThrow(Registries.STRUCTURE).get(MillagerStructures.STRONG_ROOM_KEY)
                ...
                return head.is(MillagerItems.ILLAGER_HEAD.asItem());
            }
            return false;
        }));
        this.targetSelector
                .addGoal(
                        ...
                        new NearestAttackableTargetGoal<>(
                                this, Mob.class, 5, false, false, livingEntity -> {
    ...
    public void addAdditionalSaveData(@NonNull CompoundTag output) {
        ...
        if (this.getPersistentAngerTarget() != null) {
            output.putUUID("AngryAt", this.getPersistentAngerTarget());
        }
        if (this.professionOrderOwner != null) {
            output.putUUID("ProfessionOrderOwner", this.professionOrderOwner);
        }
    ...
    public void readAdditionalSaveData(@NonNull CompoundTag input) {
        ...
        this.setRemainingPersistentAngerTime(input.getInt("AngerTime"));
        if (input.hasUUID("AngryAt")) {
            this.setPersistentAngerTarget(input.getUUID("AngryAt"));
        }
        if (input.hasUUID("ProfessionOrderOwner")) {
            this.professionOrderOwner = input.getUUID("ProfessionOrderOwner");
        }
        ...
        if (input.hasUUID("SquadId")) {
            ...
            this.squadId = input.getUUID("RaidReinforcementSquadId");
        }
        this.squadLeader = input.contains("SquadLeader")
                ? input.getBoolean("SquadLeader")
                : input.getBoolean("RaidReinforcementLeader");
        this.squadLeaderBanner = input.getBoolean("SquadLeaderBanner");
        this.raidReinforcementArrived = input.getBoolean("RaidReinforcementArrived");
    }
    ...
    public boolean shouldIgnoreRetaliationAgainst(Player player) {
    ...
    @Override
    public void setPersistentAngerTarget(@Nullable UUID entityReference) {
    ...
    public void setProfessionOrderOwner(@Nullable UUID owner) {
    ...
    private void mergeNearbySquad() {
        ...
        AbstractMillager other = this.level().getEntitiesOfClass(AbstractMillager.class,
                        this.getBoundingBox().inflate(SQUAD_MERGE_RADIUS), candidate -> candidate != this
                                && candidate.isAlive() && candidate.isSquadLeader() && !squadId.equals(candidate.getSquadId())
                                && this.hasMatchingSquadCenter(candidate)).stream()
                .min(Comparator.comparingDouble(this::distanceToSqr)).orElse(null);
        if (other == null || other.getSquadId() == null) return;
        UUID otherId = other.getSquadId();
        var members = this.level().getEntitiesOfClass(AbstractMillager.class,
                this.getBoundingBox().inflate(SQUAD_LEADER_SEARCH_RADIUS), candidate -> squadId.equals(candidate.getSquadId()) || otherId.equals(candidate.getSquadId()));
        if (members.size() > MAX_SQUAD_SIZE) return;
        AbstractMillager leader = this.squadLeaderBanner != other.squadLeaderBanner
                ? this.squadLeaderBanner ? this : other
                : this.getHealth() > other.getHealth() || this.getHealth() == other.getHealth()
                && this.getUUID().compareTo(other.getUUID()) < 0 ? this : other;
