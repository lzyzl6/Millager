...
public class StrongRoomPiece extends TemplateStructurePiece {
    ...
    private static final ResourceLocation STRUCTURE = ResourceLocation.fromNamespaceAndPath(MOD_ID, "strong_room");

    public StrongRoomPiece(StructurePieceSerializationContext ctx, CompoundTag tag) {
        super(MillagerStructures.STRONG_ROOM_PIECE.get(), tag, ctx.structureTemplateManager(), id -> makeSettings(tag.contains("rot") ? Rotation.valueOf(tag.getString("rot")) : Rotation.NONE));
    }
