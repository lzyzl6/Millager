...
public class CommandPostPiece extends TemplateStructurePiece {
    ...
    private static final TagKey<Biome> FORGE_IS_DESERT =
            TagKey.create(Registries.BIOME, Identifier.fromNamespaceAndPath("forge", "is_desert"));

    private static final TagKey<Biome> HAS_DESERT_PYRAMID =
            TagKey.create(Registries.BIOME, Identifier.fromNamespaceAndPath("minecraft", "has_structure/desert_pyramid"));

    private static final Identifier[] STRUCTURES_NORMAL = new Identifier[]{
            Identifier.fromNamespaceAndPath(MOD_ID, "command_post/classic")
    };
    private static final Identifier[] STRUCTURES_DESERT = new Identifier[]{
            Identifier.fromNamespaceAndPath(MOD_ID, "command_post/sand")
    };
    private static final Identifier[] STRUCTURES_RUINED = new Identifier[]{
            Identifier.fromNamespaceAndPath(MOD_ID, "command_post/ruined_classic")
    };
    private static final Identifier[] STRUCTURES_DESERT_RUINED = new Identifier[]{
            Identifier.fromNamespaceAndPath(MOD_ID, "command_post/ruined_sand")
    };
    ...
    public CommandPostPiece(StructurePieceSerializationContext ctx, CompoundTag tag) {
        super(MillagerStructures.COMMAND_POST_PIECE.get(), tag, ctx.structureTemplateManager(),
                id -> makeSettings(tag.read("rot", Rotation.CODEC).orElseThrow()));
        this.ruined = tag.getBooleanOr("ruined", false);
        this.embedDepth = tag.getIntOr("embed_depth", 1);
    }
    ...
    public CommandPostPiece(StructureTemplateManager manager, Identifier identifier, BlockPos pos, Rotation rotation, boolean bl, int i) {
