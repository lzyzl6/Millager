...
public class LiquorCauldronHelper {
    ...
    private static ItemInteractionResult startBrewing(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, ItemStack stack) {
        if (!state.hasProperty(LayeredCauldronBlock.LEVEL) || state.getValue(LayeredCauldronBlock.LEVEL) < 3) return ItemInteractionResult.PASS_TO_DEFAULT_BLOCK_INTERACTION;
        player.swing(hand);
        if (!world.isClientSide()) {
