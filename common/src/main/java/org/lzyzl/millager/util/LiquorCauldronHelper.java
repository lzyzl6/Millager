...
public class LiquorCauldronHelper {
    ...
    public static void registerLiquorInteraction() {
        ...
        liquorIngredients.forEach(item -> CauldronInteraction.WATER.put(item, new CauldronInteraction() {
            public @NonNull InteractionResult interact(@NonNull BlockState state, @NonNull Level world, @NonNull BlockPos pos, @NonNull Player player, @NonNull InteractionHand hand, @NonNull ItemStack stack) {
                return startBrewing(state, world, pos, player, hand, stack);
            }
        }));

        CauldronInteraction.EMPTY.put(Items.GLASS_BOTTLE, new CauldronInteraction() {
            public @NonNull InteractionResult interact(@NonNull BlockState state, @NonNull Level world, @NonNull BlockPos pos, @NonNull Player player, @NonNull InteractionHand hand, @NonNull ItemStack stack) {
    ...
    private static InteractionResult startBrewing(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, ItemStack stack) {
        ...
        if (!world.isClientSide()) {
            ...
            Display.ItemDisplay display = EntityType.ITEM_DISPLAY.create(world);
            if (display != null) {
